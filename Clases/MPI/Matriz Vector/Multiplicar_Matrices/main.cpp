...
int main(int argc, char * argv[]) {
    ...
    int numeroProcesadores,idProceso,psize;
      long **A,         // Matriz a multiplicar
            ...
            *comprueba;  // Guarda el resultado final (calculado secuencialmente), su valor debe ser igual al de 'y'
    ...
    //MPI_Comm_size(MPI_COMM_WORLD, &psize); // Obtenemos el valor de nuestro identificador
    cout << "Hay " << numeroProcesadores << " procesos en el comunicador global";

    MPI_Comm_rank(MPI_COMM_WORLD, &idProceso);

    A = new long *[numeroProcesadores]; // Reservamos tantas filas como procesos haya
    x = new long [numeroProcesadores]; // El vector sera del mismo tamanio que el numero de procesadores
